...
**2026-09-14 — Prestige added (`71ec8d5`, corrected the same day in `fb6b4c2`).** Third Komodo Rips tile, `komodo-prestige` / `/images/packs/komodo-prestige.png` (1086 x 1448, same 3:4 crop as the other two), with an 8-card example checklist stored `verbatimEntries: true`, no cert numbers, and **no `finalizedOn`**. It first shipped with `finalizedOn: '2026-08-31'`, which made it the one Komodo example rendering the finalized statement and, by the notice invariant, no Whatnot caveat and no no-financial-advice line; that contradicted the `b60529f` ruling that an example is not a closed production run, so the date was dropped the same day. Purity, Legend and Prestige are all illustrative, there is still no Komodo series in the archive, and the Pokemon line renders one group, Example Checklists, with three examples, each showing the Komodo caveat and nothing else. Notice distribution is now **5 illustrative / 0 finalized / 19 none** across 24 static series; the finalized statement and the `Example checklist. N packs, 1 card per pack.` structure line render nowhere today.

**2026-09-16 — Utopia added (`44ac52b`).** Fourth Komodo Rips tile, `komodo-utopia` / `/images/packs/komodo-utopia.png` (1086 x 1448, same 3:4 crop as the other three), named the bare `Utopia` per the catalog's Komodo naming comment and placed after Purity. Its example checklist follows Prestige as corrected in `fb6b4c2`: 8 entries stored `verbatimEntries: true`, no cert numbers, **no `finalizedOn`**, appended after Prestige. Positions 1-8 are the owner-supplied order (the field is a displayed value rank), with Fusion Strike #271 Gengar VMAX at 1 and Paldea Evolved #203 Magikarp at 8. Two cards repeat from other examples (Paldean Fates #234 Charizard ex, 151 #200 Blastoise ex), which is fine for illustrative samples. The Pokemon line's Example Checklists group now lists four examples, each showing only the Komodo caveat, which was left unchanged. `scripts/test-card-api-adapter.ts` now expects four Komodo examples, checks Utopia's positions and illustrative notice, and asserts the notice split **6 illustrative / 0 finalized / 19 none** across 25 static series.

**2026-09-16 — Nova added (`06e34b2`).** Fifth Komodo Rips tile, `komodo-nova`, named the bare `Nova` and inserted alphabetically (Legend, Nova, Prestige, Purity, Utopia). It has **no art of its own**: it reuses `/images/packs/shackpack-nova.png`, the same file as the ShackPack sports Nova tile, deliberately not copied or renamed, and a comment on the entry says so. That file is 1054 x 1492, not the 1086 x 1448 (3:4) of the other Komodo art; the tile frame is `aspect-[2/3]` with `object-cover`, so it renders without layout change. The example checklist follows Utopia: 8 entries stored `verbatimEntries: true`, no cert numbers, **no `finalizedOn`**, appended after Utopia, positions in the owner-supplied order (Lost Origin #186 Giratina V at 1, 151 #204 Giovanni's Charisma at 8). The Example Checklists group now lists five examples, each with only the Komodo caveat. `scripts/test-card-api-adapter.ts` expects five Komodo examples, checks Nova's positions and illustrative notice, and asserts **7 illustrative / 0 finalized / 19 none** across 26 static series. Its `byName` and `noticeFor` lookups are now **brand-scoped** (`brandId` + `seriesName`) because ShackPack also sells a product named Nova; no existing lookup was ambiguous before the change (ShackPack's archive series is `Nova Series 1`, and the sports Nova is a catalog tile, not a checklist series), so this guards the future rather than fixing a live collision.
