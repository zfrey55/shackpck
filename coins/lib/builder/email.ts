 ...
 * Throws if email can't be sent — caller handles errors.
 */
export async function sendBuildInquiryEmail(data: BuildInquiryEmailData): Promise<void> {
  ...
  const linesHtml = data.lines
    .map((line) => {
      ...
      const coinLabel = coin?.label ?? line.coinType;
      ...
      return `
        <tr>
          ...
          <td style="padding:8px;border:1px solid #e5e7eb;">${escapeHtml(coinLabel)}</td>
  ...
  const summaryRow = `
    <tr style="background:#f3f4f6;">
      <td style="padding:8px;border:1px solid #e5e7eb;" colspan="5">
        <strong>Pack count:</strong> ${data.packCount}
        &nbsp;·&nbsp;<strong>Coins listed:</strong> ${totalCoins}
        ${unassigned !== 0 ? `&nbsp;·&nbsp;<strong style="color:#b45309;">Unassigned slots:</strong> ${unassigned}` : ''}
      </td>
    </tr>
  `;
        ...
        <table style="width:100%;border-collapse:collapse;font-size:14px;">
          <thead>
            <tr style="background:#fff7ed;">
              ...
              <th style="padding:8px;border:1px solid #e5e7eb;text-align:left;">Coin</th>
              <th style="padding:8px;border:1px solid #e5e7eb;text-align:left;">Grader</th>
              <th style="padding:8px;border:1px solid #e5e7eb;text-align:left;">Target tier</th>
  ...
  const text = [
    ...
    ...data.lines.map((line) => {
      ...
      const tier = TIER_DEFS[line.tier as Tier];
      const tierLabel = tier ? `${tier.label} (${tier.range})` : line.tier;
      return `  ${line.quantity}x ${coinLabel} — ${line.grader} — ${tierLabel}${line.notes ? ` — ${line.notes}` : ''}`;
    }),
    '',
  ...
  try {
    await sgMail.send({
      ...
      subject: `[ShackPack Builder] ${data.name} — ${data.packCount} packs — ${data.customerName}`,
      html,
      text,
    });
  } catch (err: unknown) {
