...
3. **Test Label Generation**:
   - Place a test order; labels are generated by `/api/orders`. (The
     `/api/test-fedex` endpoint was removed — it was public and
     unauthenticated.)
...
3. **Test Email Sending**:
   - Submit the contact form, which sends through `/api/contact`. (The
     `/api/test-email` endpoint was removed — it could send mail to any
     address.)
   - Check server logs
